...
class InnerProductDecoder(nn.Module):
    """Decoder for using inner product for prediction of the RECONSTRUCTION ADJACENCY MATRIX."""
...
class Model(nn.Module):
    ...
    def reparameterize(self, mu, logvar):
        ...
        if self.training:
            std = torch.exp(logvar)  # Isnt it var instead of std?
            eps = torch.randn_like(std) # alternatively   eps = torch.cuda.FloatTensor(std.size()).normal_() 
            return eps.mul(std).add_(mu)
        else:
            return mu

    def forward(self, x, adj):
        mu, logvar, feat_x = self.encode(x, adj) # mean, standard deviation, and gene features
        gnn_z = self.reparameterize(mu, logvar) # reparametrized graph output features
        z = torch.cat((feat_x, gnn_z), 1) # encoder and graph variational autoencodes feature concatenation for gene matrix reconstruction
        de_feat = self.decoder(z) # decoded gene matrix reconstruction
