...
# ====== Graph construction
def graph_computing(adj_coord, cell_num, params):
    edgeList = []
    for node_idx in range(cell_num):
        tmp = adj_coord[node_idx, :].reshape(1, -1)
        distMat = distance.cdist(tmp, adj_coord, params.knn_distanceType)
...
def graph_construction(adj_coord, cell_N, params):
    adata_Adj = graph_computing(adj_coord, cell_N, params)
